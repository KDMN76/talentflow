...
export async function register(
  ...
  return withAuthTx(async (client: PoolClient) => {
    ...
    // Create the tenant-creating user as 'owner' (niet 'admin'): de
    // SYSTEM_ROLES-matrix (lib/permissions.ts) sluit `users:admin` bewust uit
    // van de 'admin'-rol (user-/rolbeheer vereist een striktere rol dan
    // tenant-admin). 'owner' heeft wél de volledige matrix en is exact de rol
    // waar `requireRole('admin', 'owner')` overal in de codebase al naar
    // verwijst — zonder deze fix kon GEEN gebruiker ooit custom-rollen of
    // security-settings beheren.
